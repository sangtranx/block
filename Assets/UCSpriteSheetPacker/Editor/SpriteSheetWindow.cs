...
namespace SpriteSheetProject
{
    ...
    public class SpriteSheetWindow : EditorWindow
    {
        ...
        public static void CreateSpriteAnimationByOnceClick()
        {
            ...
            foreach (var subPath in Directory.GetDirectories(path, "*", SearchOption.AllDirectories))
            {
                var fileName = Path.GetFileName(subPath);
                var fileSheet = Directory.GetParent(subPath).FullName.Replace('\\', '/') + "/" + fileName + "." + format_1;
                var fileAnimation = Directory.GetParent(subPath).FullName.Replace('\\', '/') + "/" + fileName + "." + format_2;
                animationPath = fileAnimation;
                SpriteSheetPacker.GenerateSS(subPath, fileSheet, fileName);
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Button("Gen By Once Click", Contents.buttonStyle))
            {
                CreateSpriteAnimationByOnceClick();
            }
            GUILayout.BeginHorizontal();
